...
//======================================================================================================================
	...
	Dialog.prototype = {
		...
		appendDom : function(dom){//此处dom应为拼接好的字符串；
			...
			var timer = setTimeout(function(){
				if($(t.theMask) && $(t.theMask).length > 0){
					if($(t.theMask).mCustomScrollbar){
				...
				if($(t.theAlertBox).length && $(t.theAlertBox).length > 0){
					if($(t.theAlertBox).find('.alertBody').mCustomScrollbar){
	...
	PopWin.prototype = {
		...
		appendDom : function(str){
			...
			var timer = setTimeout(function (){
				//优化滚动条
				if($('#' + t.winId).find('.left-box').length > 0 ){
					if($('#' + t.winId).find('.left-box').mCustomScrollbar){
				...
				if($('#' + t.winId).find('.middle-box').length > 0){
					if($('#' + t.winId).find('.middle-box').mCustomScrollbar){
